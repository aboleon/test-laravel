...
class DashboardOrdersWithoutPecByGroupQuery
{
    use EventModelTrait;
...
WITH group_totals AS (
    SELECT
        ...
        SUM(CASE WHEN ocs.id IS NOT NULL THEN (ocs.total_net + ocs.total_vat) ELSE 0 END) AS services_total,
        ...
        SUM(CASE WHEN oca.id IS NOT NULL THEN (oca.total_net + oca.total_vat) ELSE 0 END) AS accom_total,
        ...
        SUM(CASE WHEN oct.id IS NOT NULL THEN (oct.amount_net + oct.amount_vat) ELSE 0 END) AS taxroom_total
    FROM
        orders o
    INNER JOIN
    ...
    GROUP BY
        pt.group
    HAVING
        (services_total_net + accom_total_net + taxroom_total_net) > 0
)
