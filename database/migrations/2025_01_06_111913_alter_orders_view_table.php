...
SELECT `o`.`id`                                                                                                   AS `id`,
       ...
       case
           ...
           when 'orator' = `o`.`client_type`
               then 'Orateur' end                                                                                 AS `client_type_display`,
       ...
       case
           ...
           when 'unpaid' = `o`.`status`
               then 'Non-soldée' end                                                                              AS `status_display`,
       `oi`.`invoice_number`                                                                                      AS `invoice_number`,
       case
           when 'group' = `o`.`client_type` then `g`.`name`
           when `o`.`client_type` in ('contact', 'orator')
       ...
       concat(date_format(`o`.`created_at`, '%d '), case
                                                        when date_format(`o`.`created_at`, '%m') = '01'
                                                            then 'Janvier'
...
from ((((((((`orders` `o` left join `order_invoices` `oi`
          ...
          on (`o`.`client_type` in ('contact', 'orator') and
        ...
        on (`o`.`client_type` in ('contact', 'orator') and `u`.`id` = `ec`.`user_id` and
