...
SELECT
    ...
    CASE
        ...
        WHEN `o`.`client_type` COLLATE utf8mb4_unicode_ci IN ('contact', 'orator') THEN CONCAT(`u`.`last_name`, ' ', `u`.`first_name`)
        END AS `name`,
...
FROM
    `orders` `o`
        ...
        LEFT JOIN `events_contacts` `ec` ON (`o`.`client_type` IN ('contact', 'orator') AND `u`.`id` = `ec`.`user_id` AND `o`.`event_id` = `ec`.`event_id`)
