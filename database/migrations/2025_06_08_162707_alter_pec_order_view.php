...
select `pd`.`id` AS `pec_distribution_id`,
       ...
       format(sum(case when `pd`.`type` = 'transport' then 0 else coalesce(`pd`.`total_net`,0) end) / 100.0,2) AS `total_net`,
       format(sum(case when `pd`.`type` = 'transport' then 0 else coalesce(`pd`.`total_vat`,0) end) / 100.0,2) AS `total_vat`,
       format(sum(case when `pd`.`type` = 'transport' then coalesce(`pd`.`unit_price`,0) else coalesce(`pd`.`total_net`,0) + coalesce(`pd`.`total_vat`,0) end) / 100.0,2) AS `total`,
       case when `pd`.`type` = 'uncategorized' then 'Non catégorisé'
       ...
       case when `pd`.`type` = 'service' then json_unquote(json_extract(`ess`.`title`,'$.fr'))
            ...
            when `pd`.`type` = 'transport' then case when `pd`.`shoppable_id` = 1 then 'Divine Id' when `pd`.`shoppable_id` = 2 then 'Participant' when `pd`.`shoppable_id` = 3 then 'Non demandé' else NULL end end AS `shoppable`,
...
left join (select distinct `account_address`.`user_id` AS `user_id`,`account_address`.`country_code` AS `country_code` from `account_address`) `aa` on(`u`.`id` = `aa`.`user_id`))
left join `countries` `c` on(`aa`.`country_code` = `c`.`code`))
left join (select distinct `account_profile`.`user_id` AS `user_id`,`account_profile`.`domain_id` AS `domain_id` from `account_profile`) `ap` on(`u`.`id` = `ap`.`user_id`))
left join `dictionnary_entries` `de_domain` on(`ap`.`domain_id` = `de_domain`.`id`))
group by `pd`.`id`,`pd`.`type`,`pd`.`grant_id`,`pd`.`order_id`,`ec`.`event_id`,concat(`u`.`first_name`,' ',`u`.`last_name`),`pd`.`shoppable_id`
having sum(case when `pd`.`type` = 'transport' then coalesce(`pd`.`unit_price`,0) else coalesce(`pd`.`total_net`,0) + coalesce(`pd`.`total_vat`,0) end) <> 0

UNION ALL

select `ed`.`id` AS `pec_distribution_id`,
       'grantdeposit' AS `pec_distribution_type`,
       `ed`.`shoppable_id` AS `grant_id`,
       `ed`.`order_id` AS `order_id`,
       `ec`.`event_id` AS `event_id`,
       `ed`.`event_contact_id` AS `event_contact_id`,
       concat(`u`.`first_name`,' ',`u`.`last_name`) AS `participant`,
       format(coalesce(`ed`.`total_net`,0) / 100.0,2) AS `total_net`,
       format(coalesce(`ed`.`total_vat`,0) / 100.0,2) AS `total_vat`,
       format((coalesce(`ed`.`total_net`,0) + coalesce(`ed`.`total_vat`,0)) / 100.0,2) AS `total`,
       'Caution Grant' AS `type`,
       NULL AS `shoppable`,
       json_unquote(json_extract(`c`.`name`,'$.fr')) AS `country`,
       json_unquote(json_extract(`de_domain`.`name`,'$.fr')) AS `domain`,
       json_unquote(json_extract(`eg`.`title`,'$.fr')) AS `grant`
...
left join `users` `u` on(`ec`.`user_id` = `u`.`id`))
left join `event_grant` `eg` on(`ed`.`shoppable_id` = `eg`.`id`))
left join (select distinct `account_address`.`user_id` AS `user_id`,`account_address`.`country_code` AS `country_code` from `account_address`) `aa` on(`u`.`id` = `aa`.`user_id`))
left join `countries` `c` on(`aa`.`country_code` = `c`.`code`))
left join (select distinct `account_profile`.`user_id` AS `user_id`,`account_profile`.`domain_id` AS `domain_id` from `account_profile`) `ap` on(`u`.`id` = `ap`.`user_id`))
left join `dictionnary_entries` `de_domain` on(`ap`.`domain_id` = `de_domain`.`id`)
where `ed`.`shoppable_type` = 'grantdeposit'
  and `ed`.`status` in ('paid','billed')
  and (coalesce(`ed`.`total_net`,0) + coalesce(`ed`.`total_vat`,0)) <> 0
        ");
    }
