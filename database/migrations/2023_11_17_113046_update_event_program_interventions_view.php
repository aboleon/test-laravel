...
SELECT
    ...
    DATE_FORMAT(event_program_day_rooms.datetime_start, '%d/%m/%Y') AS date,
    ...
    JOIN event_program_day_rooms ON s.event_program_day_room_id = event_program_day_rooms.id
    LEFT JOIN dictionnary_entries ON i.specificity_id = dictionnary_entries.id
    LEFT JOIN place_rooms AS r ON i.place_room_id = r.id
    LEFT JOIN events_contacts_interventions AS eci ON eci.event_program_intervention_id = i.id
    LEFT JOIN events_contacts AS c ON c.id = eci.events_contacts_id
    LEFT JOIN users AS u ON u.id = c.user_id
    GROUP BY i.id, event_program_day_rooms.datetime_start, i.start, i.end, s.name, i.name, dictionnary_entries.name, r.name;
