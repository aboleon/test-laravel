...
return new class extends Migration
{
    ...
    public function up(): void
    {
       ...
       CASE
           WHEN `ec`.`pec_enabled` = 1 THEN 'Activée'
           ELSE NULL END                                         AS `pec_enabled_display`,
       CASE
           WHEN `ec`.`is_pec_eligible` = 1 THEN 'Eligible'
           ELSE NULL END                                         AS `pec_eligible_display`,
       ...
       (SELECT COUNT(0)
        FROM (`event_sellable_service` JOIN `event_contact_sellable_service_choosables` ON (
            `event_sellable_service`.`id` =
            `event_contact_sellable_service_choosables`.`choosable_id`))
        WHERE `ec`.`id` = `event_contact_sellable_service_choosables`.`event_contact_id`
          AND `event_sellable_service`.`deleted_at` IS NULL) + (SELECT COUNT(0)
                                                                         ...
                                                                         WHERE `ec`.`id` =
                                                                               `event_program_session_moderators`.`events_contacts_id`) +
       (SELECT COUNT(0)
        FROM `event_program_intervention_orators`
        WHERE `ec`.`id` = `event_program_intervention_orators`.`events_contacts_id`) + (SELECT COUNT(0)
                                                                                                 FROM `event_transports`
...
FROM (((((((((((`events_contacts` `ec` JOIN `users` `u`
                ON (`u`.`id` = `ec`.`user_id`)) JOIN `events` `e`
