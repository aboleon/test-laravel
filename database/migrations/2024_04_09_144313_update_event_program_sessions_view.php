...
SELECT
    ...
    CASE
        WHEN p.name IS NOT NULL THEN
            CONCAT(DATE_FORMAT(dr.datetime_start, '%d/%m/%Y'), ' - ',  p.name, ' > ', JSON_UNQUOTE(JSON_EXTRACT(pr.name, '$.fr')))
       ELSE
            CONCAT(DATE_FORMAT(dr.datetime_start, '%d/%m/%Y'), ' - ',  drp.name, ' > ', JSON_UNQUOTE(JSON_EXTRACT(drpr.name, '$.fr')))
    END AS container,
...
LEFT JOIN place_rooms drpr ON drpr.id = dr.room_id
LEFT JOIN places drp ON drp.id = drpr.place_id
