...
return new class extends Migration
{
    ...
    CASE
        ...
        ELSE NULL
    END AS pec_enabled_display,
    CASE
        WHEN ec.is_pec_eligible = 1 THEN 'Eligible'
        ELSE NULL
    END AS pec_eligible_display,
    ...
    (SELECT COUNT(0)
     ...
     JOIN event_contact_sellable_service_choosables ON event_sellable_service.id = event_contact_sellable_service_choosables.choosable_id
...
GROUP BY
    ec.id, e.id, u.id, u.first_name, u.last_name, u.email,
    ...
    CASE
        ...
        WHEN pt.group = 'industry' THEN 'Industriels'
    END,
    json_unquote(json_extract(pt.name, '$.fr'))
        ");
    }
