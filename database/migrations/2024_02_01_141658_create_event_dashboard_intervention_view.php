...
SELECT
    ...
    COALESCE(ec1.id, ec2.id) AS event_contact_id,
    ...
    CASE 
        WHEN FLOOR(epi.duration / 60) > 0 THEN 
            CONCAT(LPAD(FLOOR(epi.duration / 60), 2, '0'), 'h', LPAD(epi.duration % 60, 2, '0'), 'm')
        ELSE 
            CONCAT(epi.duration % 60, 'm') 
    END AS duration_formatted,
    IF(epio.events_contacts_id IS NOT NULL, 'Orateur',
        IF(epsm.events_contacts_id IS NOT NULL, JSON_UNQUOTE(JSON_EXTRACT(de.name, '$.fr')),
            NULL)
    ) AS type,
    JSON_UNQUOTE(JSON_EXTRACT(epi.name, '$.fr')) AS title,
    JSON_UNQUOTE(JSON_EXTRACT(eps.name, '$.fr')) AS session
FROM
    event_program_interventions epi
    JOIN event_program_sessions eps ON epi.event_program_session_id = eps.id
    LEFT JOIN event_program_intervention_orators epio ON epio.event_program_intervention_id = epi.id
    LEFT JOIN events_contacts ec1 ON epio.events_contacts_id = ec1.id
    LEFT JOIN event_program_session_moderators epsm ON epsm.event_program_session_id = eps.id
    LEFT JOIN events_contacts ec2 ON epsm.events_contacts_id = ec2.id
    LEFT JOIN dictionnary_entries de ON epsm.moderator_type_id = de.id
