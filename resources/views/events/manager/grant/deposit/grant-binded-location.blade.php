...
<div id="grant-binded-locations" class="grant_deposit">
    ...
    <div class="row align-items-center">
        <div class="col-sm-10">
            <x-mfw::google-places :geo="new \MetaFramework\Models\Dummy\Address()" :params="['types' => ['locality','country']]" field="sellable_grant_binded" placeholder="Tapez le nom d'une localité ou un pays"/>
        </div>
        <div class="col-sm-2">
    ...
    <table id="grant-binded-saved-locations" class="table text-dark table-bordered mt-2">
        <thead>
        <tr>
            <th style="width: 40%;">Localité</th>
            <th colspan="2" style="width: 40%;">Pays</th>
        </tr>
        </thead>
        <tbody>
        @if ($data->id)
...
<template id="grant-binded-location-row">
    <x-grant-binded-location :location="new \App\Models\EventManager\Grant\GrantDepositLocation()"/>
</template>
        ...
        function deleteGrantBindedLocation() {
            $('.delete_grand_binded_row').off().on('click', function () {
                ...
                if (id.length < 1 || isNaN(id)) {
                    ...
                    ajax('action=removeGrantBindedLocationRow&id=' + Number(id) + '&identifier=' + identifier, $('#grant_locations_messages'));
                }
            });
        }
    </script>
